...
struct MainGame {
    ...
    MainGame(std::shared_ptr<PlayerState> ps) : player_state(ps) {
        GameState &gs = *ps->gs;
        ...
        cursor_atlas_position = Rectangle{.x = 168, .y = 108, .width = 11, .height = 15};
        ...
        BeginDrawing();
        {
            ...
            BeginMode3D(camera); 
            {
                ...
                for(const auto coords : to_render) {
                    ...
                    if (hx.tileid != -1 && hx.getFractionVisibility(ps.fraction) != HexData::Visibility::NONE) {
                        DrawModelEx(as.resourceStore.m_hex_table.at(hx.tileid).model, Vector3{tx, -0.2, ty}, Vector3{0, 1, 0}, 0.0, Vector3{scale, scale, scale}, tint);
                    }
            ...
            if (as.debug) {
                ...
                if (hovered_tile.tileid != -1) {
                    DrawText(as.resourceStore.m_hex_table.at(hovered_tile.tileid).name.c_str(), 10, 50, 20, BLACK);
                }
            }
