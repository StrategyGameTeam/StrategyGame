...
Textbox::Textbox(const std::string& arg_text, float arg_x, float arg_y, float arg_width, float arg_height, float arg_font_size) :
	...
	text_size(MeasureTextEx(GetFontDefault(), arg_text.c_str(), arg_font_size, 1.f)),
...
void Textbox::set_text(const std::string &arg_text) {
	this->text = arg_text;
	this->text_size = MeasureTextEx(GetFontDefault(), arg_text.c_str(), font_size, 1.f);
	box.width = text_size.x;
}
...
void Textbox::draw() const
{
	...
	DrawTextPro(GetFontDefault(), text.c_str(), Vector2{box.x, box.y},
		Vector2{text_size.x * 0.5f, text_size.y * 0.5f}, 0.f, font_size, 1.f, BLACK);
}
